...
# ==============================================================================
# ADVERSARIAL SYSTEM WITH RECOVERY (Constructed Base — required substrate)
# ==============================================================================
...
class AdversarialSystem:
    ...
    def auto_recovery_tick(self, recovery_rate: Union[int, float]) -> str:
        ...
        achieved. Identical logic to recover_stability but framed as automatic
        homeostasis rather than active intervention.
        """
        ...
        if recovery_rate < 0:
            raise ValueError(
                ...
                "(damaging events are handled by inject_threat, not auto_recovery_tick)."
            )
        ...
        if (
            ...
            return (
                ...
                f"Restored +{actual_restored:.1f} stability ({self.stability:.1f}/{self.max_stability})."
            )
        ...
        return (
            ...
            f"Current: {self.stability:.1f}/{self.max_stability}."
        )
...
#     exceed its original design capacity without structural modification.
#
# Boundary —
#     · If threat_magnitude or recovery_rate is negative or non-numeric,
#       ValueError/TypeError halts execution immediately.
...
#       actual_restored becomes zero — idempotent "no change" narrative.
#
# Equilibrium —
#     After inject_threat: BALANCED, STRAINED, or COLLAPSED.
#     After auto_recovery_tick: COLLAPSED (blocked), STRAINED (partial), or
#     BALANCED (rebalanced). No undefined intermediate states.
# ==============================================================================


# ==============================================================================
# AUTOMATED EQUILIBRIUM LOOP
# ==============================================================================

class EquilibriumLoop:
    """
    Orchestrates a continuous while-loop where each tick applies a random
    adversarial attack followed by an automatic recovery event, modeling open
    systems under persistent environmental stress with passive homeostasis.
    """

    # [SINGULARITY_HEADER]
    # Intent  : Model an open system under continuous environmental stress with
    #           passive homeostatic repair, revealing the dynamic tug-of-war
    #           between degradation and restoration over an unpredictable lifespan.
    # State   : Loop holds [AdversarialSystem, threat_range, recovery_rate,
    #           tick_delay, max_ticks, current_tick, audit_log]; expects
    #           [execution trigger].
    # Action  : Validate config → initialize system → enter while loop → generate
    #           random threat → inject threat → append audit → check collapse →
    #           apply auto-recovery → append audit → increment tick → delay →
    #           check max_ticks → return complete forensic record.
    ...
    def __init__(
        ...
        # --(validation: enforce simulation Assumption 1 — well-ordered non-negative bounds)--> [State: threat bounds verified]
        ...
        # --(validation: enforce simulation Assumption 2 — non-negative restorative energy)--> [State: recovery rate verified]
        if recovery_rate < 0:
            raise ValueError(
        ...
        # --(validation: enforce simulation Assumption 4 — at least one tick observed)--> [State: tick budget verified]
        if max_ticks < 1:
            raise ValueError(
                "[COGNITION_GAP]: max_ticks must be >= 1."
            )
    ...
    def run(self) -> Tuple[List[str], str, float, int]:
        """
        Execute the continuous equilibrium loop and return the complete audit
        trail, final state, final stability, and the tick at which termination
        occurred.
        """
        ...
        self.audit_log.append(
            f"AUTOMATED EQUILIBRIUM ENGINE | Max Stability: {self.system.max_stability:.1f} | "
            f"Threshold: {self.system.threshold:.1f} | Recovery: {self.recovery_rate:.1f}/tick"
        )
        self.audit_log.append(
            "=========================================================="
        )

        # [Entity: system state flag, max_ticks limit, and current_tick counter]
        # --(loop guard: enforce termination when terminal state reached or safety budget exhausted)--> [State: while condition evaluated]
        while (
            self.system.system_state != "COLLAPSED"
            and self.current_tick < self.max_ticks
        ):
            ...
            # --(stochastic sampling: model unpredictable adversarial intensity for current tick)--> [State: threat_force generated]
            ...
            threat_result = self.system.inject_threat(threat_force)

            # [Entity: threat result narrative]
            # --(audit capture: preserve causal outcome in ordered record)--> [State: threat result appended to audit_log]
            self.audit_log.append(f"   └── {threat_result}")

            # [Entity: system state flag post-threat]
            # --(emergency evaluation: detect if threat triggered collapse to bypass recovery)--> [State: termination condition assessed]
            if self.system.system_state == "COLLAPSED":
                # [Entity: collapsed system]
                # --(loop break: enforce immediate termination since dead systems cannot process recovery)--> [State: loop exited, no recovery executed]
                break
            ...
            recovery_result = self.system.auto_recovery_tick(self.recovery_rate)
            ...
            self.audit_log.append(f"   └── {recovery_result}")
            ...
            # [Entity: current_tick and max_ticks parameters]
            # --(safety evaluation: detect if tick budget exhausted without collapse)--> [State: timeout condition assessed]
            if self.current_tick >= self.max_ticks:
                # [Entity: max_ticks limit]
                # --(timeout break: prevent infinite loop in degenerate configurations)--> [State: loop exited with survival outcome]
                self.audit_log.append(
                    f"\n[TIMEOUT]: Reached max_ticks ({self.max_ticks}) without collapse."
                )
                break
        ...
        self.audit_log.append(
            f"💀 SIMULATION TERMINATED AT TICK T+{self.current_tick}"
        )
        self.audit_log.append(
            f"Final System State: {self.system.system_state} | "
        ...
        # [Entity: finalized audit_log, terminal state, remaining stability, and termination tick]
...
# ==============================================================================
# LAYER 3: GRAVITY CHECK (EquilibriumLoop)
# ==============================================================================
# Origin —
#     This loop exists because real systems do not experience isolated attacks
#     followed by planned recovery phases. They exist in open environments where
#     stress and repair occur simultaneously and continuously. The while-loop
#     form captures the dynamic equilibrium of sustained conflict: each tick
#     is a microcosm of the full adversarial-recovery cycle, and the net
#     trajectory over many ticks reveals whether the system's homeostasis is
#     sufficient to survive its environment or whether cumulative damage
#     eventually overwhelms repair capacity. The random threat generation
#     models environmental unpredictability; the fixed recovery rate models
#     passive structural resilience (e.g., biological healing, automatic
#     failover, thermal dissipation).
#
# Boundary —
...
#     · If the system collapses during a threat phase, the loop breaks
#       immediately — recovery is bypassed, no post-collapse processing.
#     · If max_ticks is reached without collapse, the loop terminates with a
#       timeout message — the system survived the campaign.
...
#       4. The termination tick integer.
...
#     providing a forensic trail. In collapse, the log reveals the exact tick
#     at which failure occurred. In survival (timeout), the log shows the full
#     max_ticks trajectory. There are no ambiguous outcomes — the terminal
#     condition is always either collapse or survival timeout.
# ==============================================================================
...
if __name__ == "__main__":
    ...
    # --(instantiation: create loop with 100.0 max stability, 10.0 threshold, [15,95] threat range, 12.0 recovery)--> [State: loop configured and ready]
    loop = EquilibriumLoop(
        max_stability=100.0,
    ...
    # [Entity: EquilibriumLoop instance]
    # --(execution: run continuous campaign and capture forensic output)--> [State: simulation completed, audit trail returned]
    audit_log, final_state, final_stability, termination_tick = loop.run()
    ...
    print(f"\n>>> OPERATOR SUMMARY: State={final_state} | Stability={final_stability:.1f}/{loop.system.max_stability:.1f} | Ticks={termination_tick}")